  ...
  switch (hookType) {
    ...
    case "SubagentStop": {
      ...
      if (matchingSub) {
        ...
        // Extract model + per-model token usage from the subagent's transcript.
        // agent_transcript_path points to the subagent's own JSONL file.
        if (data.agent_transcript_path) {
          const subSummary = transcriptCache.extractSubagentSummary(data.agent_transcript_path);
          if (subSummary) {
            stmts.updateAgentModel.run(subSummary.primaryModel, matchingSub.id);
            for (const [model, tokens] of Object.entries(subSummary.tokensByModel)) {
  ...
  if (data.transcript_path) {
    ...
    if (result) {
      ...
      if (tokensByModel) {
        ...
        // Mirror the dominant model onto the main agent row so the UI can
        // show which model the session is running without a separate lookup.
        const currentMain = mainAgent || getMainAgent(sessionId);
        if (currentMain) {
          let bestModel = null;
          let bestOutput = -1;
          for (const [model, tokens] of Object.entries(tokensByModel)) {
            if (tokens.output > bestOutput) {
              bestOutput = tokens.output;
              bestModel = model;
            }
          }
          if (bestModel && bestModel !== currentMain.model) {
            stmts.updateAgentModel.run(bestModel, currentMain.id);
